...
class NGaussians(Dataset):
    def __init__(self,N=2, moments=None,inter_distance=4.0,max_cols=None,max_examples=None,scaling=0.5):
        """
        ...
        if moments is not None:
            ...
            loc = pt.stack([x[0] if pt.is_tensor(x[0]) else pt.tensor(x[0]) for x in moments])
            scale = pt.stack([x[1] if pt.is_tensor(x[1]) else pt.tensor(x[1]) for x in moments])
        else:
            if max_cols is None:
        ...
        comp=pt.distributions.Independent(pt.distributions.Normal(loc,scale),1)
...
if __name__=="__main__":
    ...
    ds=NGaussians()
    plt.scatter(ds.examples[:,0],ds.examples[:,1]);plt.show()
    ds=NGaussians(N=16)
    plt.scatter(ds.examples[:,0],ds.examples[:,1]);plt.show()
